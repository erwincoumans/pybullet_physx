...
static void convertURDFToVisualShape2(const UrdfShape* visual, const char* urdfPathPrefix, const btTransform& visualTransform, btAlignedObjectArray<GLInstanceVertex>& verticesOut, btAlignedObjectArray<int>& indicesOut, btAlignedObjectArray<MyTexture3>& texturesOut, b3VisualShapeData& visualShapeOut, struct CommonFileIOInterface* fileIO, int flags)
{
	...
	switch (visual->m_geometry.m_type)
	{
		...
		case URDF_GEOM_SPHERE:
		{
			...
			glmesh->m_numvertices = sizeof(textured_detailed_sphere_vertices) / strideInBytes;
			glmesh->m_vertices->resize(glmesh->m_numvertices);
			
			for (int i = 0; i < glmesh->m_numvertices; i++)
			{
				GLInstanceVertex& vtx = glmesh->m_vertices->at(i);
				vtx.xyzw[0] = textured_detailed_sphere_vertices[i * 9 + 0]*radius;
				vtx.xyzw[1] = textured_detailed_sphere_vertices[i * 9 + 1]*radius;
				vtx.xyzw[2] = textured_detailed_sphere_vertices[i * 9 + 2]*radius;
				vtx.xyzw[3] = 0;
	...
	if (linkPtr)
	{
		...
		for (int v1 = 0; v1 < cnt; v1++)
		{
...

			{
				...
				if (shapeIndex < 0 && vertices.size() && indices.size())
				{
				...
				if (shapeIndex>=0)
				{
					graphicsIndex = m_data->m_instancingRenderer->registerGraphicsInstance(shapeIndex, &visualShape.m_localVisualFrame[0], &visualShape.m_localVisualFrame[3], &visualShape.m_rgbaColor[0], scaling);
				}
